...
void ATestCArrayReplication::Server_TearDownImpl()
{
	...
	{
		UE_LOG(LogSpatialGDKTestSuite, Log, TEXT("TestCase %s: Unable to tear down dynamically created actor C style array"), *TestName);
	}
...
void ATestCArrayReplication::ValidateReplication_Client()
{
	...
	check(StablyNamedArray[0]->GetPathName() == TEXT("/Script/GDKTestGyms.Default__TestUObject"));

	check(StablyNamedArray[1]->IsA(UTestUObject::StaticClass()));
	check(StablyNamedArray[1] == UTestUObject::StaticClass()->GetDefaultObject());
	check(StablyNamedArray[1]->GetPathName() == TEXT("/Script/GDKTestGyms.Default__TestUObject"));
