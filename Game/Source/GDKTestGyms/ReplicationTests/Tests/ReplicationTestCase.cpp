...
void AReplicationTestCase::Tick(float DeltaTime)
{
	...
	if (bRunning)
	{
		...
		UE_LOG(LogSpatialGDKTestSuite, Log, TEXT("TestCase: %s: Test complete!"), *TestName);
		bIsFinished = true;
		bRunning = false;
	}
}
...
{
	UE_LOG(LogSpatialGDKTestSuite, Log, TEXT("TestCase %s: Test started!"), *TestName);
...
void AReplicationTestCase::SignalReplicationSetup()
{
	...
	UE_LOG(LogSpatialGDKTestSuite, Log, TEXT("TestCase: %s : Replication setup on Server"), *TestName);
}
...
void AReplicationTestCase::SignalResponseRecieved()
{
	...
	CurrentRunnerIndex++;
	SetupNewGDKRunner();
	UE_LOG(LogSpatialGDKTestSuite, Log, TEXT("TestCase %s: Response RPC recieved from a client"), *TestName);
}

void AReplicationTestCase::SendServerRPCs()
{
	UE_LOG(LogSpatialGDKTestSuite, Log, TEXT("TestCase %s: Validating replication"), *TestName);

	ValidateClientReplicationImpl();

	UE_LOG(LogSpatialGDKTestSuite, Log, TEXT("TestCase %s: Replication successful on client, sending response RPC"), *TestName);
