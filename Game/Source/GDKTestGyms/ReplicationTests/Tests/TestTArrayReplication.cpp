...
void ATestTArrayReplication::Server_StartTestImpl()
{
	...
	UTestUObject* StablyNamedObject = LoadObject<UTestUObject>(nullptr, TEXT("/Script/GDKTestGyms.Default__TestUObject"));
...
void ATestTArrayReplication::Server_TearDownImpl()
{
	...
	for (ATestActor* Actor : DynamicallyCreatedArray)
	{
		...
		{
			UE_LOG(LogSpatialGDKTestSuite, Log, TEXT("TestCase %s: Unable to tear down dynamically created actor in TArray"), *TestName);
		}
	}
														...
														const TArray<TEnumAsByte<EnumNamespace::EUnrealTestEnum>>& TestUEnumTArray)
{
	...
	check(TestStablyNamedArray[0]->GetPathName() == TEXT("/Script/GDKTestGyms.Default__TestUObject"));
	...
	check(TestStablyNamedArray[1]->GetPathName() == TEXT("/Script/GDKTestGyms.Default__TestUObject"));
