...
void ADynamicLBSInfo::IncreseActorCounter_Implementation(const uint32 TargetWorkerId)
{
	uint32 ActorCounter;
...
void ADynamicLBSInfo::DecreaseActorCounter_Implementation(const uint32 TargetWorkerId)
{
	uint32 ActorCounter;
	if (TargetWorkerId > (uint32)ActorCounters.Num())
	{
...
void ADynamicLBSInfo::UpdateWorkerCells_Implementation(const TArray<FBox2D>& NewWorkerCells)
{
	this->DynamicWorkerCells = NewWorkerCells;
}
